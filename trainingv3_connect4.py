...
def eval_action_mask(env_fn, num_games=100, render_mode=None, **env_kwargs):
    ...
    print(
        f"Starting evaluation between two trained agents."
    )
    ...
    model_1 = MaskablePPO.load(latest_policy)
    model_2 = MaskablePPO.load(latest_policy)  # Assuming the same policy for now
...
if __name__ == "__main__":
    ...
    train_action_mask(env_fn, steps=20_480, seed=42, **env_kwargs)

    # Evaluate 100 games between two trained agents
    eval_action_mask(env_fn, num_games=100, render_mode=None, **env_kwargs)

    # Watch two games between two trained agents
