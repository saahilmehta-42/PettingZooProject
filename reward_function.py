...
class SB3ActionMaskWrapper(pettingzoo.utils.BaseWrapper):
    ...
    def step(self, action):
        ...
        # Modify the reward function to encourage blocking the opponent
        if reward == 0 and not termination and not truncation:
            # Check for potential opponent win and reward blocking
            for opp_action in range(self.action_space.n):
                if self._is_potential_win(self.agent_selection, opp_action):
                    reward += 1  # Add a reward for blocking opponent

        return obs, reward, termination, truncation, info
    
    def _is_potential_win(self, agent, action):
        """Check if the given action by the opponent could lead to a win."""
        # Implement logic to determine if an action leads to a potential win
        # This is a simplified example; a full implementation will require board state analysis
        # You would need to analyze the board and check for potential winning moves
        return False
...
def train_action_mask(env_fn, steps=100_000, seed=0, **env_kwargs):
...
if __name__ == "__main__":
    ...
    train_action_mask(env_fn, steps=2_000_480, seed=34, **env_kwargs)
